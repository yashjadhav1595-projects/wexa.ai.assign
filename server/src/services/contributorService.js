...
const { mockContributors } = require('../utils/mockGraphData');
...
class ContributorService {
  async getAllContributors() {
    ...
    try {
      ...
      logger.warn(`[ContributorService] DB error: ${err.message}. Falling back to mock contributors.`);
      return mockContributors;
    }
  }
  ...
  async getContributorById(id) {
    if (!driver) {
      const found = mockContributors.find(c => c.id === id || c.username === id);
      if (found) {
        return {
          ...found,
          projects: [{ name: 'graph-guard-core', role: 'core-maintainer', commits: 142 }],
          orgs: [{ name: found.org, role: found.role, since: 2024 }],
          issues: [{ title: 'feat: add ReBAC graph traversal index' }],
          following: []
        };
      }
      return mockContributors[0];
    }
    try {
      const cypher = `
